...
class DocumentIngestionPipeline:
    ...
    def __init__(self, config: IngestionConfig, documents_folder: str = "documents", clean_before_ingest: bool = False, use_streaming: bool = True):
        ...
        # Policy: costruire SEMPRE il grafo in produzione o se esplicitamente richiesto via env
        always_build_graph_env = os.getenv("ALWAYS_BUILD_GRAPH", "0").lower() in ("1", "true", "yes")
        is_production_env = os.getenv("APP_ENV", "development").lower() == "production"
        self.build_graph = (not getattr(self.config, "skip_graph_building", False)) or always_build_graph_env or is_production_env
    ...
    async def initialize(self):
        if not self._initialized:
            ...
            # Inizializza Graph solo se abilitato
            if self.build_graph and self.graph_builder is not None:
    ...
    async def _clean_databases(self):
        ...
        # Pulisce Neo4j solo se il grafo è abilitato
        if self.build_graph:
            try:
