...
-- Create document ingestion status tracking table
...
-- Add foreign key to documents table for tracking
ALTER TABLE documents ADD COLUMN IF NOT EXISTS ingestion_status_id INTEGER REFERENCES document_ingestion_status(id);

-- Add metadata columns to documents for structured citations
ALTER TABLE documents ADD COLUMN IF NOT EXISTS category VARCHAR(100);
ALTER TABLE documents ADD COLUMN IF NOT EXISTS document_order INTEGER;
ALTER TABLE documents ADD COLUMN IF NOT EXISTS priority_weight INTEGER DEFAULT 100;

-- Add metadata columns to chunks for better tracking
ALTER TABLE chunks ADD COLUMN IF NOT EXISTS category VARCHAR(100);
ALTER TABLE chunks ADD COLUMN IF NOT EXISTS document_order INTEGER;

-- Indexes for ingestion tracking
CREATE INDEX idx_ingestion_file_path ON document_ingestion_status(file_path);
...
-- Indexes for documents metadata
...
-- Trigger for updating document_ingestion_status timestamp
