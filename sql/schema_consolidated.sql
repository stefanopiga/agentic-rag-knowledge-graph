-- =====================================================
-- SCHEMA CONSOLIDATO: FUSIONE PRODUCTION + EXTENSION
-- =====================================================
-- Consolidazione di:
-- - schema_with_auth.sql (production multi-tenant completo)
-- - section_tracking_schema.sql (extension per section tracking)
-- Depreca: schema.sql (legacy, non multi-tenant)
-- 
-- Versione: 3.0 - Schema Consolidato
-- Compatible con Django, deployabile su Neon PostgreSQL
-- =====================================================

-- ⚠️  DEPRECATION NOTICE:
-- Questo schema sostituisce i seguenti file:
-- - sql/schema.sql (DEPRECATED - legacy schema senza multi-tenancy)
-- - sql/schema_with_auth.sql (SOSTITUITO - ora integrato qui)
-- - sql/section_tracking_schema.sql (SOSTITUITO - ora integrato qui)
-- 
-- Non utilizzare più i file sopra elencati. Utilizzare solo schema_consolidated.sql
-- per tutti i nuovi deployment e per gli upgrade da schema legacy.
...
DROP VIEW IF EXISTS failed_sections CASCADE;
...
DROP TRIGGER IF EXISTS trigger_update_document_sections_timestamp ON document_sections;
...
DROP FUNCTION IF EXISTS cleanup_failed_sections CASCADE;
DROP FUNCTION IF EXISTS update_document_sections_timestamp CASCADE;
...
-- =====================================================
-- SECTION TRACKING SYSTEM (from extension)
-- =====================================================

-- Tabella per tracking sezioni individuali (da section_tracking_schema.sql)
...
-- Indici ottimizzati per section tracking
CREATE INDEX idx_document_sections_tenant ON document_sections(tenant_id);
CREATE INDEX idx_document_sections_status ON document_sections(document_status_id, status);
CREATE INDEX idx_document_sections_position ON document_sections(document_status_id, section_position);
CREATE INDEX idx_document_sections_failed ON document_sections(status) WHERE status IN ('failed', 'processing');

-- Trigger per aggiornamento automatico timestamp section tracking
...
-- =====================================================
-- SECTION TRACKING FUNCTIONS (from extension)
-- =====================================================

-- Funzione per cleanup sezioni fallite (da section_tracking_schema.sql) 
-- Ora con supporto multi-tenant
...
-- Vista per recovery rapido (da section_tracking_schema.sql) con multi-tenancy
...
WHERE ds.status IN ('failed', 'processing')
  AND ds.tenant_id = dis.tenant_id -- Ensure tenant isolation
...
CREATE POLICY section_isolation_policy ON document_sections FOR ALL USING (true);
...
-- ✅ Consolidation of schema_with_auth.sql + section_tracking_schema.sql
-- ✅ Deprecation notices for legacy schema.sql
...
-- Consolidates: schema_with_auth.sql + section_tracking_schema.sql
-- Deprecates: schema.sql (legacy)
